...
from pydantic_containers import ValidatedDict, ValidatedList
from pydantic_containers._mapping import ValidatedDefaultDict


def test_mapping() -> None:
    class T(BaseModel):
        d: ValidatedDict[int, ValidatedList[float]] = Field(default_factory=dict)
...
def test_default_dict() -> None:
    class T(BaseModel):
        d: ValidatedDefaultDict[int, ValidatedList[float]] = Field(default_factory=dict)
        model_config = ConfigDict(validate_default=True)

    t = T()
    t.d["1"].append("7.5")  # type: ignore
    assert 1 in t.d
    assert t.d[1] == [7.5]
